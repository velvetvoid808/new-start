...
(function () {
    ...
    function updateVolumeControl(control, valueLabel, volume) {
        if (control) {
            ...
            control.setAttribute("aria-valuetext", `${volume} percent`);
        }
    ...
    function playSoundEffect(kind) {
        ...
        try {
            ...
            // At the 50% default, all sound styles are intentionally clear
            // enough to hear without needing to push the device volume high.
            const volume = (soundEffectsVolume / 100) * 0.4;
    ...
    function startMusic() {
        ...
        if (selectedTrack === "night" && backgroundMusic) {
    ...
    function resetAllSettings() {
        ...
        if (settingsResetStatus) {
            settingsResetStatus.textContent = "Settings restored to their defaults.";
        }
    }
    ...
    function getSoundEffectKind(interactiveElement) {
        ...
        if (
            interactiveElement.matches(
                "[data-card-shape], [data-button-shape], [data-site-theme], [data-motion-preference], [data-desktop-navigation], [data-music-track], [data-sound-style]"
            )
        ) {
            return "select";
        }
