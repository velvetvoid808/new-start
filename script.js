...
(function () {
    ...
    function getMusicPreference() {
        try {
            return window.localStorage.getItem(musicPreferenceKey) === "on";
        } catch (error) {
            return false;
        }
    }
    ...
    function saveShapePreference(preferenceKey, shape) {
        try {
            window.localStorage.setItem(preferenceKey, shape);
        } catch (error) {
            // The selected look remains active until the current page closes.
        }
    }
    ...
    function applyTextSize() {
        ...
        if (textSizeControl) {
            ...
            textSizeControl.setAttribute(
                ...
                textSize === 0
                    ? "Current text size"
                    : `${Math.round(scale * 100)} percent of the current text size`
            );
        }
    ...
    function updateAppearanceChoices(buttons, selectedShape) {
        buttons.forEach(function (button) {
            const buttonShapeValue =
                button.dataset.cardShape || button.dataset.buttonShape;
            const isSelected = buttonShapeValue === selectedShape;
    ...
    function applyDesktopNavigation() {
        document.body.dataset.desktopNavigation = desktopNavigation;
    ...
    function updateMusicSwitch() {
        ...
        if (musicSwitchState) {
            musicSwitchState.textContent = musicEnabled ? "ON" : "OFF";
        }
    }
    ...
    function startMusic() {
        ...
        if (playback) {
            playback.catch(function () {
                // Playback can only begin from the visitor's Music switch click.
            });
        }
    }
    ...
    cardShapeButtons.forEach(function (button) {
        button.addEventListener("click", function () {
            cardShape = button.dataset.cardShape === "rounded"
                ? "rounded"
                : "square";
            saveShapePreference(cardShapePreferenceKey, cardShape);
            applyCardShape();
        });
    });
    ...
    buttonShapeButtons.forEach(function (button) {
        button.addEventListener("click", function () {
            buttonShape = button.dataset.buttonShape === "rounded"
                ? "rounded"
                : "square";
            saveShapePreference(buttonShapePreferenceKey, buttonShape);
            applyButtonShape();
        });
    });
    ...
    desktopNavigationButtons.forEach(function (button) {
        button.addEventListener("click", function () {
            desktopNavigation =
                button.dataset.desktopNavigation === "collapsed"
                    ? "collapsed"
                    : "expanded";
